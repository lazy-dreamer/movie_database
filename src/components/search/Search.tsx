...
import {
  resetSearchQuery,
  setSearchQuery,
} from "../../redux/search_slice/searchSlice";
import fetchSearchFilms from "../../redux/thunks/fetchSearchFilms";

export function Search() {
  const [state, setState] = useState("");
  const dispatch: AppDispatch = useDispatch();
  const { searchQuery, films, status } = useSelector(
  ...
  return (
    <section>
      <div className="screen_content">
        ...
        {status === "loading" ? (
          <Preloader customClass="" />
        ) : (
          <div className="quarter_blocks">
            {films.map((item: TFilm) => (
              <FilmBlock key={item.id} film={item} />
            ))}
            {searchQuery.length > 1 && films.length === 0 && (
              <h3 className="subtitle">Nothing found :(</h3>
            )}
          </div>
        )}
      </div>
